    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <div className="flex items-center gap-8">
          {/* Brand */}
          <Link href="/" className="text-lg font-bold tracking-tight text-gray-900 hover:opacity-90">
            ForGesty
          </Link>
          ...
          <nav className="hidden items-center gap-6 text-sm md:flex">
            ...
            {/* (Optionnel) Conserver la page Présentation mais ne pas l’afficher si tu ne veux pas */}
            {/* <Link href="/presentation" className="text-gray-700 hover:text-indigo-600">Découvrir</Link> */}
          </nav>
        </div>

        {/* Right: Compte */}
        <div className="relative" ref={menuRef}>
          {/* If NOT logged in: show “Se connecter” */}
          {!session?.user ? (
            <Link
              href="/auth/login"
